...
az network vnet subnet create -g $RESOURCE_GROUP --vnet-name $VNET_NAME -n $KV_SUBNET --address-prefixes  --nsg $NSG_NAME
...
az network public-ip create -g $RESOURCE_GROUP -n bastionip --sku Standard
az network bastion create -g $RESOURCE_GROUP -n corebastion --vnet-name $VNET_NAME --public-ip-address bastionip
...
#Create Private ACR
az acr create -g $RESOURCE_GROUP -n acrcore --sku Premium --default-action Deny --public-network-enabled true